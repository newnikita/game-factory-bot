...
2) МОБИЛЬНОЕ УПРАВЛЕНИЕ ОНЛАЙН: Размер Canvas должен быть адаптивным. ОБЯЗАТЕЛЬНО реализуй наэкранные полупрозрачные HTML-кнопки (виртуальный джойстик, стрелки или кнопки действий), которые будут располагаться поверх Canvas в нижней части экрана. Они должны быть привязаны к событиям touchstart и touchend. Игра должна полноценно управляться как с клавиатуры, так и с экрана мобильного устройства.
...
bot.on('text', async (ctx) => {
    if (ctx.session?.step === 'awaiting_ai_prompt') {
        ...
        const msg = await ctx.reply('✨ Призываю мощности Gemini 2.5 Pro... \n\nПроцесс запущен в фоновом режиме. Генерация сложной логики может занять время. Просто подожди, я обновлю это сообщение! ⏳');
